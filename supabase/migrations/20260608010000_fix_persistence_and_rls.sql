...
-- Drop any restrictive auth.uid() policies that may exist (profiles is known to exist)
...
-- NOTE: DROP POLICY IF EXISTS still fails if the TABLE doesn't exist,
-- so all policy drops must be inside the table-existence check.
